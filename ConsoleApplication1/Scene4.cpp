...
const Sphere s_floor = Sphere(1e5, Vec(W / 2, -1e5 + 25, 50), Vec(0, 0, 0), Vec(.6, .6, .6), false, true, 0);
...
bool light3_on = false && lightAll_on;

Vec light1_position = Vec(W + 100, H, 0);
...
Vec light3_position = Vec(W, H, 0);	//Light3 : p0 : light3_position v : light1_direction (Point Light)
...
#if 0
	//conflict test balls
	sphere.push_back(Sphere(25, Vec(650, 680, 50), Vec(0, 1, 0), Vec(.7, .8, .7), true, true, 50, false, true));
#endif
	...
	if (root != NULL) {
		...
		if (light1_on){
			if (meetLight1(root->touchPoint, 0.5)){		//120 degree
				...
				//				double distance = min(1.0, 500 / sqrt(between1.dot(between1) + 0.6));

				//				root->color = root->color * distance;
				root->color = root->color * 1.2;
			}
			else{
		...
		//Light3 : point light
		if (light3_on){
			if (meetLight3(root->touchPoint)){		//360 degree
				//				Vec between3 = light3_position - root->touchPoint;
				//				double distance = min(1.0, 300 / sqrt(between3.dot(between3) + 0.5));

				//				root->color = root->color * distance;
				root->color = root->color * 1.2;
			}
			else{
				root->color = root->color * 0.6;
			}
		}
...
#ifdef MODE_CAPTURE
	string filename = "y" + to_string(time) + ".png";
...
void timePass(){
	...
	if (time == 300){
		sphere.pop_back();
		sphere.push_back(Sphere(25, Vec(650, 680, 50), Vec(-0.1, 1, -0.1), Vec(1, 0, 0), true, true, 50, false, true));
	...
	if (time == 600){
		sphere.pop_back();
		sphere.push_back(Sphere(25, Vec(650, 680, 50), Vec(0.1, 1, -0.1), Vec(0, 0, 1), true, true, 50, false, true));
	...
	if (time == 900){
		sphere.pop_back();
		sphere.push_back(Sphere(25, Vec(650, 680, 50), Vec(-0.1, 1, -0.1), Vec(0, 1, 0), true, true, 50, false, true));
	...
	if (time == 1500){
		sphere.pop_back();
		sphere.push_back(Sphere(25, Vec(650, 680, 50), Vec(0, 1, -0.1), Vec(1, 1, 0), true, true, 50, false, true));
	...
	//moving
	for (int i = 1; i < sphere.size() - 1; ++i){		//except greatest ball, floor
		sphere[i].move();
	}

	//innerBall part
	for (int i = 1; i < num_innerBall + 1; ++i){		//except greatest ball
		if (sphere[i].speed == 0)
			continue;
	...
	//outerBall part
	for (int k = num_innerBall + 1; k < sphere.size() - 1; ++k){
		sphere[k].checkConflict(&sphere[0], false);
