...
const double effect_t = 0.3;

const Sphere s_floor = Sphere(1e5, Vec(W / 2, -1e5 + 50, 300), Vec(0, 0, 0), Vec(.6, .6, .6), false, true, 0);
...
bool light1_on = true && lightAll_on;
bool light2_on = true && lightAll_on;
bool light3_on = true && lightAll_on;

Vec light1_position = Vec(CAPTURE_W / 2, CAPTURE_H / 2, 100);
Vec light1_direction = Vec(CAPTURE_W / 2, CAPTURE_H / 2, 300) - light1_position;	//Light1 : p0 : light1_position v : light1_direction (Spot Light)
Vec light2_position = Vec(-2 * W + 1900, 2 * H, 100);		//Light2 : from light2_position to Origin (Directional Light)
Vec light3_position = Vec(CAPTURE_W / 2, CAPTURE_H / 2, 0);	//Light3 : p0 : light3_position v : light1_direction (Point Light)
Vec viewer = Vec(CAPTURE_W / 2, CAPTURE_H, -1000);
...
void makeSphere(){
	...
	const int RR = 125;
	//answer ball
	sphere.push_back(Sphere(RR, Vec(600 - RR, 500, 100), Vec(0, 0, 0), Vec(1, 0, 0), true, true));	//R
	sphere.push_back(Sphere(RR, Vec(700 + RR, 500, 100), Vec(0, 0, 0), Vec(0, 0, 1), true, true));	//B
	sphere.push_back(Sphere(RR, Vec(600 - RR, 175, 100), Vec(0, 0, 0), Vec(0, 1, 0), true, true));	//G
	sphere.push_back(Sphere(RR, Vec(700 + RR, 175, 100), Vec(0, 0, 0), Vec(1, 0, 0), true, true));	//R
#endif

	//floor
	sphere.push_back(s_floor);
	sphere.push_back(Sphere(1e5, Vec(W / 2, 200, 1e5 + 500), Vec(0, 0, 0), Vec(.6, .6, .6), false, true, 0));

}
void makePolygon(){
	Vertices.push_back(Vec(-50, 51, 150));
	Vertices.push_back(Vec(250, 53, 150));
	Vertices.push_back(Vec(150, 52, -150));
	Vertices.push_back(Vec(150, 551, 150));

	Faces.push_back(Face(0, 1, 3, Vec(.1, .1, .1), true, true));	//back
	Faces.push_back(Face(0, 2, 1, Vec(.1, 0, 0), true, true));	//floor
	Faces.push_back(Face(2, 3, 0, Vec(0, .1, 0), true, true));	//left
	Faces.push_back(Face(3, 2, 1, Vec(0, 0, .1), true, true));	//right

}
bool meetLight1(Vec touchPoint, double cosine_, bool isPolygon){
	Vec direction = (light1_position - touchPoint).norm();
	touchPoint = isPolygon ? touchPoint + (Vec(0.01, 0.01, 0.01).mult(direction)) : touchPoint - (Vec(0.01, 0.01, 0.01).mult(direction));
	...
	for (int s = 0; s < Faces.size(); ++s){
		double t = Faces[s].intersect(ray);
		if (t > 0)
			return false;
	}
...
bool meetLight2(Vec touchPoint, bool isPolygon){
...
bool meetLight3(Vec touchPoint, bool isPolygon){
...
void postorder(RayTracing *root) {
	if (root != NULL) {
		...
		if (light1_on){
			...
			double sqr_d = between1.dot(between1) / 100000;
			double r = min(1 / sqrt(sqr_d), 1.1);

			root->color = root->color * max(0.0000001, r);
			if (!meetLight1(root->touchPoint, 0.5, root->isPolygon)){		//120 degree
...
//			}
		...
		if (light2_on){
			if (!meetLight2(root->touchPoint, root->isPolygon))
				root->color = root->color * 0.7;
		}
		//Light3 : point light
		if (light3_on){
//			if (meetLight3(root->touchPoint, root->isPolygon)){
				Vec between3 = light3_position - root->touchPoint;
				double sqr_d = between3.dot(between3) / 160000;
				double r = min(1 / sqrt(sqr_d), 10);

				root->color = root->color * max(0.000001, r);
//			}
//			else{
//				root->color = root->color * 0.1;
//			}
		}
...
void makingTree(RayTracing *root, Ray ray, int depth, int transException, bool transEx){
	Vec touch_point;
	Vec N;
	Vec R;
	bool isReflec = false;
	...
	if (depth < DEPTH){
		...
		for (int p = 0; p < Faces.size(); ++p){
			if (transEx && p == transException)
				continue;

			t = Faces[p].intersect(ray);
			if (t > 0 && min_t > t){		//if intersect && is not the exception
				min_t = t;
				min_p = p;
				isPolygon = true;
			}
		...
		if (min_p != -1){
			root->isPolygon = isPolygon;
			if (isPolygon)
				root->color = Faces[min_p].color;
			else{
				root->color = sphere[min_p].color;
				root->transparent = sphere[min_p].isTransparent;
			}
			...
			if (isPolygon){
				N = Faces[min_p].getNormalV();
			}
			else{
				N = touch_point - sphere[min_p].C;
				N = N.norm();
			}
			...
			isReflec = Faces[min_p].isReflec;

			//transmission
			root->transmission = new RayTracing(touch_point);
			isTrans = Faces[min_p].isTrans;
			current_Exception = min_p;
		}

		if (isReflec)
			makingTree(root->reflection, Ray(isPolygon? touch_point + R * 0.01 : touch_point, R), depth + 1, -1, isPolygon);
		if (isTrans)
			makingTree(root->transmission, Ray(touch_point - N * 2, ray.v), depth + 1, current_Exception, isPolygon);
	}
}
...
#pragma omp parallel for schedule(dynamic)
	for (int i = 0; i < W; ++i){
		for (int j = 0; j < H; ++j){
			...
			makingTree(raytracing_start, Ray(viewer, (Vec(i, j, 0) - viewer).norm()), 0, -1, false);
